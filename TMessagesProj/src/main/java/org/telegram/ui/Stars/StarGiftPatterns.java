...
//        -105.66f, -88.33f, 18, .1487f
    ...
    public static void drawProfilePattern(Canvas canvas, Drawable pattern, float w, float h, float alpha, float full, float targetX, float targetY) {
        if (alpha <= 0.0f) return;

        final float fraction = alpha;
        ...
        for (int level = 0; level < 2; level++) {
            for (int i = 0; i < 8; i++) {
                float localRadius;
                float t;
                t = fraction == 0f ? 0f : (float) Math.pow(2, (i + 2) * fraction) / (float) Math.pow(2, i + 2);
                ...
                float x = (float) (targetX + localRadius * Math.cos(angleRadians));
    ...
    public static void drawProfilePattern(Canvas canvas, Drawable pattern, float w, float h, float alpha, float full) {
        ...
        for (int i = 0; i < profileRight.length; i += 4) {
            ...
            pattern.setBounds(
                    (int) (c + dpf2(x) - dpf2(size) / 2.0f),
                    (int) (t + dpf2(y) - dpf2(size) / 2.0f),
                    (int) (c + dpf2(x) + dpf2(size) / 2.0f),
                    (int) (t + dpf2(y) + dpf2(size) / 2.0f)
            );
            pattern.setAlpha((int) (0xFF * alpha * thisAlpha));
